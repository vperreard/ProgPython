...
import sys
import threading
from modules.invoices.factures_db_utils import (
...
class AnalyseFactures:
    ...
    def on_recharger_fichiers(self, tree, forcer_reanalyse=False, mettre_a_jour_listes_filtres=None):
        ...
        # Mettre à jour les listes des filtres si une fonction est fournie
    ...
    def on_verifier_nouvelles_factures(self, mettre_a_jour_listes_filtres=None):
        ...
        def task():
            # Vérifier les nouvelles factures
            self.verifier_nouvelles_factures()
            
            # Mise à jour des filtres si une fonction est fournie
            if mettre_a_jour_listes_filtres:
                mettre_a_jour_listes_filtres()
    ...
    def verifier_nouvelles_factures(self, mettre_a_jour_listes_filtres=None):
        """
        Vérifie la présence de nouvelles factures et gère les fichiers déplacés.
        
        Args:
            mettre_a_jour_listes_filtres (callable, optional): Fonction pour mettre à jour les listes de filtres.
        
        Returns:
            int: Nombre de nouvelles factures détectées
        """
        ...
        # Mise à jour des listes de filtres si une fonction est fournie
        if mettre_a_jour_listes_filtres:
            mettre_a_jour_listes_filtres()
...
# Filtre par montant minimum
    ...
    def creer_interface(self, parent=None):
        ...
        tree.column("fournisseur", width=120, anchor="w")
        ...
        liste_annees.bind('<<ComboboxSelected>>', lambda e: on_annee_changed())  # Add this line
        ...
        liste_mois.bind('<<ComboboxSelected>>', lambda e: on_mois_changed())  # Add this line
        ...
        # Inside the creer_interface method, modify the mettre_a_jour_listes_filtres function:
        def mettre_a_jour_listes_filtres():
            ...
            if current_year in annees:
                ...
                # Filtrer les mois disponibles pour l'année courante
                mois_annee_courante = sorted(list(set(
                    self.df_factures[self.df_factures["annee"] == current_year]["mois"].dropna().tolist()
                )))
                
                # Mettre à jour la liste des mois pour l'année courante
                if "---" not in mois_annee_courante:
                    mois_annee_courante.insert(0, "---")
                liste_mois["values"] = mois_annee_courante
                
                # Si le mois courant existe pour cette année, le sélectionner
                if current_month in mois_annee_courante:
        ...
        # Add these local function definitions inside creer_interface method:
        def on_annee_changed():
            """Gère le changement d'année dans les filtres."""
            # Réinitialiser le mois à "---" quand l'année change
            mois_var.set("---")
            ...
            if annee_selectionnee != "---":
                # Filtrer les mois pour l'année sélectionnée
                mois_disponibles = sorted(list(set(
                    self.df_factures[self.df_factures["annee"] == annee_selectionnee]["mois"].dropna().tolist()
                )))
                
                if "---" not in mois_disponibles:
                    mois_disponibles.insert(0, "---")
                
                liste_mois["values"] = mois_disponibles
            else:
                # Réinitialiser la liste des mois si "---" est sélectionné
                mois = sorted(list(set(self.df_factures["mois"].dropna().tolist())))
                if "---" not in mois:
                    mois.insert(0, "---")
                liste_mois["values"] = mois
        ...
        def on_mois_changed():
            """Gère le changement de mois dans les filtres."""
            # Appliquer les filtres automatiquement
            appliquer_filtres()

        # Then add the bind methods after creating the Combobox widgets:
        liste_annees.bind('<<ComboboxSelected>>', lambda e: on_annee_changed())
        liste_mois.bind('<<ComboboxSelected>>', lambda e: on_mois_changed())
        ...
        btn_verifier = ttk.Button(
            ...
            command=lambda: self.verifier_nouvelles_factures(mettre_a_jour_listes_filtres),
            width=25
        )
        btn_verifier.grid(row=1, column=2, padx=10, pady=5)  # Use grid instead of pack
