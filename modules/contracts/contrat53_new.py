...
def ensure_directory_exists(directory_path):
    ...
    try:
        ...
        if not os.path.exists(expanded_path):
            ...
            print(f"📁 Dossier créé: {expanded_path}")
        return expanded_path
    except Exception as e:
        print(f"❌ Erreur lors de la création du dossier {directory_path}: {str(e)}")
...
def handle_popups():
    """
    Gère les popups de Word en détectant leur apparition.
    """
    try:
        print("Surveillance des fenêtres popups...")
        ...
        # Attendre un peu pour que le premier popup apparaisse
        time.sleep(1)
        ...
        # Attendre l'apparition du deuxième popup
        time.sleep(1)
...
def perform_mail_merge(word_file, replaced_name, replacing_name, start_date, pdf_folder, contract_type="MAR"):
    ...
    if not os.path.exists(pdf_folder):
        try:
            ...
            print(f"📁 Dossier créé: {pdf_folder}")
        except Exception as e:
            print(f"❌ Erreur lors de la création du dossier {pdf_folder}: {str(e)}")
    ...
    # Utiliser le chemin complet sans tilde pour Word
    word_file_expanded = os.path.expanduser(word_file)
    
    # Script AppleScript pour ouvrir Word et le document, puis sauvegarder en PDF
    applescript = f"""
    tell application "Microsoft Word"
        activate
        delay 1
        
        open POSIX file "{word_file_expanded}"
        delay 1
        
        set activeDoc to active document
        save as activeDoc file name POSIX file "{output_pdf_path}" file format format PDF
    ...
    try:
        # 1. Lancer Word avec open (puis attendre un peu)
        subprocess.Popen(["open", word_file_expanded])
        time.sleep(0.5)  # Attendre un court instant
        
        # 2. Lancer un thread pour gérer les popups
        popup_thread = threading.Thread(target=handle_popups)
        popup_thread.daemon = True
        popup_thread.start()
        
        # 3. Attendre que le thread finisse ou atteigne le timeout
        # Un timeout de 3 secondes devrait suffire pour les deux popups
        popup_thread.join(timeout=3)
        
        # 4. Exécuter l'AppleScript pour le reste des opérations
        result = subprocess.run(["osascript", "-e", applescript], capture_output=True, text=True, check=False)
        print(f"Résultat AppleScript: {result.stdout}")
        if result.stderr:
            print(f"Erreur AppleScript: {result.stderr}")

        # 5. Vérifier si le fichier PDF a été créé
        ...
        if os.path.exists(output_pdf_path):
            ...
            # Chercher les fichiers PDF récemment créés dans le dossier cible
            pdf_files = [f for f in os.listdir(pdf_folder) if f.lower().endswith('.pdf')]
            pdf_files.sort(key=lambda x: os.path.getmtime(os.path.join(pdf_folder, x)), reverse=True)
            
            if pdf_files and (time.time() - os.path.getmtime(os.path.join(pdf_folder, pdf_files[0]))) < 10:
                alt_path = os.path.join(pdf_folder, pdf_files[0])
                print(f"✅ PDF alternatif trouvé: {alt_path}")
                return alt_path
